...
-- Move window to/from special workspaces
create_bind(vars.kbMoveWinToWsSpecial, hl.dsp.window.move({ workspace = "special:special" }))
create_bind(vars.kbMoveWinToMusicWs, hl.dsp.window.move({ workspace = "special:music" }))
create_bind(vars.kbMoveWinToCommunicationWs, hl.dsp.window.move({ workspace = "special:communication" }))
...
create_bind("SUPER + H", hl.dsp.focus({ direction = "left" }), repeating)
create_bind("SUPER + J", hl.dsp.focus({ direction = "down" }), repeating)
...
hl.define_submap("resize", function()
	...
	create_bind("K", fn.resize_active_window(0, 10), repeating)
	create_bind("L", fn.resize_active_window(10, 0), repeating)
...
hl.define_submap("move", function()
	...
	create_bind("J", hl.dsp.window.move({ direction = "down" }), repeating)
	create_bind("K", hl.dsp.window.move({ direction = "up" }), repeating)
	create_bind("L", hl.dsp.window.move({ direction = "right" }), repeating)

	-- Change focus without leaving move mode
	create_bind("SUPER + H", hl.dsp.focus({ direction = "left" }), repeating)
	create_bind("SUPER + J", hl.dsp.focus({ direction = "down" }), repeating)
	create_bind("SUPER + K", hl.dsp.focus({ direction = "up" }), repeating)
	create_bind("SUPER + L", hl.dsp.focus({ direction = "right" }), repeating)
...
create_bind(vars.kbWindowPip, function()
	...
	if active then
		local pip = fn.move_actions(active) or {}
...
create_bind({
	...
	"XF86AudioPlay",
	"XF86AudioPause",
}, hl.dsp.global("caelestia:mediaToggle"), locked)
